...
//                 //SharedCodeLogger.Debug("Sent: " + packet.Length);
//             }
//         }
...
//                 SharedCodeLogger.Debug($"Port {port} is already being used, incrementing...");
...
//                     SharedCodeLogger.Debug($"Peer {peerIP} timed out :c");
...
//                         SharedCodeLogger.Debug($"Resending packet #{outgoingPacket.index}");
...
//             //SharedCodeLogger.Debug("Got packet meta-type: " + type);

//             if (!peers.TryGetValue(remoteEndpoint, out RemotePeer peerData))
//             {
//                 SharedCodeLogger.Debug($"Communicating with new peer {remoteEndpoint}");
...
//                     SharedCodeLogger.Debug($"Peer {remoteEndpoint} terminated connection");
//                     return false;
...
//         {
//             SharedCodeLogger.Debug($"Sending acknowledge for packet #{index}");
...
//         {
//             SharedCodeLogger.Debug($"Sending all known peers a final message!");
    ...
    public class UDPPeerManager : IDisposable
    {
        ...
        public void Update() {
            ...
            for (int i = peers.Count - 1; i >= 0; i--) {
                ...
                if (peer.TicksSinceLastIncomingPacket >= NetIOPlatform.timeoutTime) {
                    peersToRemove.Add(peer);
                    continue;
                }

                peer.OutgoingPacketAcummulator += (ulong)elapsedTime;
                ulong sendAmount; sendAmount = peer.OutgoingPacketAcummulator / NetIOPlatform.heartbeatTime;
                if (sendAmount >= 1)  {
                    peer.OutgoingPacketAcummulator -= sendAmount * NetIOPlatform.heartbeatTime;
                    ...
                    for (ulong j = 0; j < sendAmount; j++) {
                        ...
                        } else if (peer.TicksSinceLastIncomingPacket > NetIOPlatform.heartbeatTime) {
                            SendRaw(
                                new byte[1] {1},
                                peer,
