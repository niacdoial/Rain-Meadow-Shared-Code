...
namespace RainMeadow.Shared
{
    ...
    public partial class SecuredPeerManager : BasePeerManager
    {
        ...
        public enum ClearPacketType: byte {
        ...
        public enum PacketType : byte {
            Unreliable_v1 = 0,
            UnreliableWithIDSig_v1,
            Reliable_v1, // and ordered!
            ReliableWithIDSig_v1,
            HeartBeat_v1,  // also serves as acknowledgement
        }
        public enum PeerAuthState : byte
        {
            None = 0,  // cleartext comms
            Unestablished,  // unknown keys
            Established,  // known comms key, unknown identity
            Identifies,  // known identity
        }
        ...
        public class RemotePeer : IDisposable {
            ...
            PeerAuthState authState = Unestablished;
            byte[] identity_pk;
            byte[] connection_pk;
            byte[] connection_computed_k;
            ...
            void IDisposable.Dispose() {
                LibSodium.sodium_memzero(this.connection_computed_k, CryptoPlatform.BOX_DERVK_SIZE);
            }
        ...
        public SecuredPeerManager(int default_port = DEFAULT_PORT, int port_attempts = FIND_PORT_ATTEMPTS) {
            InitSocket();
            this.identity_pk = new byte[LibSodium.crypto_sign_publickeybytes()];
            this.identity_sk = new byte[LibSodium.crypto_sign_secretkeybytes()];
            this.connection_pk = new byte[LibSodium.crypto_box_publickeybytes()];
            this.connection_sk = new byte[LibSodium.crypto_box_secretkeybytes()];
            this.ResetKeys();
        }
        ...
        public void SendRaw(byte[] packet, RemotePeer peer, ClearPacketType packet_type) {
            int extraLength = 1;
            switch (packet_type) {
            case ClearPacketType.Boxed_v1:
                extraLength = 1 + LibSodium.crypto_box_noncebytes();
                break;
            case ClearPacketType.BoxedWithPubKey_v1:
                extraLength = 1 + LibSodium.crypto_box_publickeybytes() + LibSodium.crypto_box_noncebytes();
                break;
            case ClearPacketType.CleartextBroadcast_v1:
                break:
            default:
                throw new Exception("unknown Cleartext Packet type... bad code update?");
            };
        ...
        void IDisposable.Dispose() {
            ...
            LibSodium.sodium_memzero(this.identity_pk, LibSodium.crypto_sign_publickeybytes());
            LibSodium.sodium_memzero(this.identity_sk, LibSodium.crypto_sign_secretkeybytes());
            LibSodium.sodium_memzero(this.connection_pk, LibSodium.crypto_box_publickeybytes());
            LibSodium.sodium_memzero(this.connection_sk, LibSodium.crypto_box_secretkeybytes());
